...
def plot_status_distribution(status_counts, output_path):
    ...
    ax1.set_title("Model Test Status Distribution", fontsize=14, fontweight='bold', pad=20)
    ...
    ax2.set_title("Model Test Success Rate", fontsize=14, fontweight='bold', pad=20)
...
def plot_answer_completeness(df, output_path):
    ...
    ax.set_ylabel("Number of Covered Dimensions (0-3)", fontsize=12)
    ax.set_title("Key Information Completeness by Question", fontsize=14, fontweight='bold', pad=20)
...
def plot_cn_en_consistency(df, output_path):
    ...
    ax.set_yticklabels([f"Q{qid}" for qid in question_ids], fontsize=10)

    # Add value labels (0/1) in each cell
    for i in range(len(question_ids)):
        for j in range(len(dimensions)):
            text = ax.text(j, i, consistency_matrix[i][j], ha="center", va="center",
                           color="white" if consistency_matrix[i][j] == 1 else "black", fontweight='bold')

    # Add color bar
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label("Consistency (1=Matched, 0=Missing)", fontsize=11)

    ax.set_title("CN-EN Answer Dimension Consistency Heatmap", fontsize=14, fontweight='bold', pad=20)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_defect_analysis(defect_counts, defect_question_counts, output_path):
    ...
    if not defect_counts.empty:
        ...
        ax1.set_title("Answer Defect Type Distribution", fontsize=14, fontweight='bold', pad=20)
    else:
        ax1.text(0.5, 0.5, "No Significant Defects", ha='center', va='center', transform=ax1.transAxes, fontsize=14)
        ax1.set_title("Answer Defect Type Distribution", fontsize=14, fontweight='bold', pad=20)
    ...
    if not defect_question_counts.empty:
        ...
        ax2.set_title("Top 3 Defect-Prone Questions", fontsize=14, fontweight='bold', pad=20)
        # Add value labels
        for i, v in enumerate(top3_defect_questions.values):
            ax2.text(v + 0.05, i, str(v), ha='left', va='center', fontweight='bold')
    else:
        ax2.text(0.5, 0.5, "No Defective Questions", ha='center', va='center', transform=ax2.transAxes, fontsize=14)
        ax2.set_title("Top 3 Defect-Prone Questions", fontsize=14, fontweight='bold', pad=20)
